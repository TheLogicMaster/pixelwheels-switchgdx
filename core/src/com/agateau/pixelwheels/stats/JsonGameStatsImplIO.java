...
//    private final Gson mGson = new GsonBuilder().setPrettyPrinting().create();
    ...
    public void load(GameStatsImpl gameStats) {
        ...
        String data = mHandle.readString("UTF-8");

        GameStatsImpl loaded = json.fromJson(GameStatsImpl.class, data);
        for (TrackStats trackStats: loaded.mTrackStats.values())
            trackStats.mGameStats = gameStats;
        gameStats.mTrackStats.putAll(loaded.mTrackStats);
        gameStats.mBestChampionshipRank.putAll(loaded.mBestChampionshipRank);
        gameStats.mEvents.putAll(loaded.mEvents);
...
//        mHandle.writeString(json, false /* append */);
        ...
        mHandle.writeString(json.toJson(gameStats), false /* append */);
    }
